...
: G4VSensitiveDetector(name){  collectionName.insert(hitsCollectionName); }
...
void SensitiveDetector::StoreData2(const G4DynamicParticle* dynamic_particle)
{
	...
	if (!file->IsExist(2, secondaries, secondaries)) file->CreateHisto(secondaries,secondaries,"proton-neutron",20,0,20,20,0,20);
